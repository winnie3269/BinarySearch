...
public int binarySearch(int catNumToFind) //uses while loop
{
  int low=0;
  ...
  while (low<=high)
  {
    guess=(low+high)/2; //important that the guess value is in the loop
    if (store[guess].getCatNum()>catNumToFind) //if guess>target
    {
      high=guess-1;
    } else if (store[guess].getCatNum()<catNumToFind) //if guess<target
    {
      low=guess+1;
    } else
    {
      return store[guess].getInventory(); //returns the correct inventory
    }
  }
  return -1;
}
public int binarySearch(int catNumToFind, int nLow, int nHigh) //uses if statements
{
